...
def get_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('-outfmt', choices=['csv', 'tsv', 'json'], default='csv',
...
def main():
    ...
    # Write/show results in TSV format
    if args.outfmt == 'tsv':
        args.out.write('seq_id\t' + '\t'.join(DOMAIN_INFO) + '\n')
        for seq_id in results:
            for domain in results[seq_id]:
                if not domain.clan:
                    domain = domain._replace(clan='No_Clan')
                domain_list = [seq_id] + list(domain)
                args.out.write('\t'.join(map(str, domain_list)) + '\n')
